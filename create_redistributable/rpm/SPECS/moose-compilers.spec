...
# set MOOSE Specific Variables
export MOOSE_PPS_WIDTH=180
export MOOSE_JOBS=\`cat /proc/cpuinfo | grep processor | wc -l\`
EOF
            # Support non-interactive login shells
            # Remove previous initailization section
            if [ -f /etc/bashrc ] && [ `cat /etc/bashrc | grep -c "START-INITIALIZE-MOOSE"` -ne 0 ]; then
                # Remove previous initialization section
                sed -i'' -e '/#START-INITIALIZE-MOOSE/,/#END-INITIALIZE-MOOSE/d' /etc/bashrc
            fi
            cat <<EOF >> /etc/bashrc
#START-INITIALIZE-MOOSE
# Source moose-environment if not already done so
if [ -z "\$MOOSE_JOBS" ] && [ -f /etc/profile.d/moose-environment.${a_shell} ]; then
  source /etc/profile.d/moose-environment.${a_shell}
fi
...
setenv MOOSE_JOBS=\`cat /proc/cpuinfo | grep processor | wc -l\`
EOF
